...
parsed.forEach(obj => {
  if (obj.id === undefined) {
    ...
    if (obj.article && typeof obj.id === 'string' && /[A-Za-z]/.test(obj.id)) {
      // nothing
    }
  }
});
...
parsed.forEach(obj => {
  if (typeof obj.id === 'string' && obj.id.endsWith('1') && obj.article && obj.article.includes('243A')) {
    obj.id = '243A';
  }
});

// Ensure unique ids; if duplicates, maintain order and keep both (they should be unique if correct)
